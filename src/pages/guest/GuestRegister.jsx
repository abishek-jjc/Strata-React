...
export default function GuestRegister() {
  ...
  async function checkCollegeRegistration(name, department) {
    ...
    try {
      ...
      if (collegeRow) {
        ...
        if (leaderRow) {
          // If the logged-in user is the registered leader, it is not a registration block
          if (sessionUser && sessionUser.email && leaderRow.email && sessionUser.email.toLowerCase() === leaderRow.email.toLowerCase()) {
            return false
          }
          return true
        }
  ...
  // Clear scanner error and reset scanner state to retry
          ...
          if (leader) {
            ...
            if (savedCol && savedDept) {
              ...
              if (!isReg) {
                ...
                setScanError(`The department "${savedDept}" from "${savedCol}" is already registered.`)
              }
            }
          }
        } catch (err) {
  ...
  useEffect(() => {
    if (loading || isDecrypted || successMessage || alreadyRegisteredLeader || scanError) return
      ...
      if (decrypted && decrypted.college && decrypted.department) {
        ...
        checkCollegeRegistration(decrypted.college, decrypted.department).then((isReg) => {
          if (!isReg) {
            ...
            setScanError(`The department "${decrypted.department}" from "${decrypted.college}" is already registered.`)
          }
        })
      } else {
  ...
  }, [loading, isDecrypted, alreadyRegisteredLeader, successMessage, scanError])
    ...
    if (location.state?.autoDecryptedCollege && location.state?.autoDecryptedDept) {
      checkCollegeRegistration(location.state.autoDecryptedCollege, location.state.autoDecryptedDept).then((isReg) => {
        if (!isReg) {
          ...
          setScanError(`The department "${location.state.autoDecryptedDept}" from "${location.state.autoDecryptedCollege}" is already registered.`)
        }
      })
    }
      ...
      <GuestLayout>
        <section className="guest-section" style={{ padding: '40px 20px' }}>
          ...
          <div className="guest-glass-panel" style={{ padding: '40px', maxWidth: '600px', margin: '0 auto', textAlign: 'center' }}>
            ...
            {scanError && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'center', margin: '15px 0' }}>
                <div style={{ color: 'var(--g-accent)', fontSize: '0.95rem', padding: '10px 15px', borderRadius: '8px', background: 'rgba(255,23,68,0.05)', border: '1px solid rgba(255,23,68,0.2)', width: '100%', boxSizing: 'border-box' }}>
                  {scanError}
                </div>
                <button 
                  type="button" 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', maxWidth: '320px', margin: '0 auto' }}>
              ...
              >
                <svg width="18" height="18" viewBox="0 0 18 18" style={{ flexShrink: 0 }}>
                  ...
                  <path d="M9 3.58c1.32 0 2.5.454 3.436 1.348l2.578-2.578C13.468 1.096 11.43 0 9 0 5.438 0 2.404 2.04 .924 4.969l3.028 2.332c.711-2.132 2.698-3.721 5.048-3.721z" fill="#EA4335"/>
                </svg>
                Sign in with Google
              </button>
            </div>
